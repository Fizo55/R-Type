...
event windowHandler::buildEvent(SDL_Event *eventSDL)
{
    ...
    if (eventType == SDL_KEYDOWN) {
        return (event(event::KEY_PRESSED, engine_math::vector2<int>(0, 0), eventSDL->key.keysym.sym));
    }
    if (eventType == SDL_KEYUP) {
        return (event(event::KEY_RELEASED, engine_math::vector2<int>(0, 0), eventSDL->key.keysym.sym));
    }
    return (event());
}
