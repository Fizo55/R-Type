...
*/
videoMode::videoMode(const engine_math::vector2<int> &size, const engine_math::vector2<int> &position, unsigned int depth, unsigned long flags)
    : size(size), position(position), depth(depth), flags(flags)
{
